...
| Дата | Что сделано |
|------|-------------|
| 2026-03-27 | **Лобби (UI):** убрано отображение и редактирование названия комнаты в `Lobby.jsx`; в шапке `PageLayout` фиксированный заголовок «Лобби»; в шаринге приглашения `roomName: null`. |
| 2026-03-27 | **Реклама перед игрой:** убран отдельный показ у хоста в `Lobby.jsx` после старта; переход в раунд для всех (включая хоста) через один эффект в `App.jsx` — реклама + `navigate(/$game)` с `/lobby`, чтобы не было рассинхрона с SDK и ситуации «у гостей реклама есть, у хоста нет». |
| 2026-03-27 | **Аналитика:** подключён счётчик Яндекс.Метрики `108270736` в `app/index.html` (script + noscript); для SPA добавлен `ym(..., 'hit', path)` на смену маршрута в `app/src/App.jsx`, чтобы корректно считались просмотры лендингов и внутренних экранов. |
| 2026-03-27 | **Лендинги игр + Premium:** исправлен `GamePromoLanding` (добавлены `primaryDisabled`/`primaryDisabledMessage`, подключён `showToast`), из-за чего снова работают переходы на `/games/*` и CTA с лендингов; в `inventory.js` убран бессрочный beta-Pro (`2099`), бесплатный Premium ограничен стартовыми **24 часами** и `hasPro` вычисляется от `proExpiresAt`. |
| 2026-03-24 | **Счетчик Манчкин (WIP):** не настольная игра, а учёт уровня/силы; лобби-карточка **неактивна** («ещё в разработке»), пресет `munchkin` убран из `VALID_LOBBY_PRESET_IDS`; лендинг `/games/munchkin`, API `munchkin/*` в `rooms.js`, экран `/munchkin` — переименование в UI на «Счетчик Манчкин». |
