...
import {
  createMafiaState,
  prepareDawn,
  applyDawnKillAnnounce,
  applyDawnDoctorAnnounce,
  applyDawnProstituteAnnounce,
  nextDawnPhaseAfterKill,
  nextDawnPhaseAfterDoctor,
  clearDawnPending,
  resolveDayVote,
  checkWin,
  toClientState,
...
export async function roomRoutes(fastify) {
  ...
  fastify.post('/rooms/:roomId/ready', async (request, reply) => {
    ...
    // Элиас: таймер и слово стартуют после «Готов» только у текущего объясняющего (см. /elias/ready-pre-round)
  ...
  function normalizeMafiaTimers(timers) {
    ...
    return {
      ...
      prepDay: toSec(t.prepDay, DEFAULT_MAFIA_PHASE_TIMERS.prepDay),
      nightMeet: toSec(t.nightMeet, DEFAULT_MAFIA_PHASE_TIMERS.nightMeet),
    };
  }
  ...
  function phaseDurationFor(gs, phase) {
    ...
    if (phase === 'prep_day_1' || phase === 'prep_day_2') return timers.prepDay || timers.day;
    if (phase === 'night_meet') return timers.nightMeet || timers.nightMafia;
    if (phase === 'dawn_kill' || phase === 'dawn_doctor' || phase === 'dawn_prostitute') return 0;
    return 0;
  }
  ...
  function finalizeMafiaRoleVotePhase(roomId, gs, io) {
    ...
    gs.killNightEnabled = false;
    markMafiaPhase(gs, 'night_meet');
    io.to(roomId).emit('mafia_phase', { phase: gs.phase });
    io.to(roomId).emit('room_updated');
    return { ok: true };
  }

  /** Завершение дневного голосования (исключение): как ручной advance с фазы voting. */
  function applyMafiaDayVotingResolution(roomId, room, gs, io) {
    const next = resolveDayVote(gs);
    Object.assign(gs, next);
    gs.killNightEnabled = true;
  ...
  fastify.post('/rooms/:roomId/mafia/role-setup-vote', async (request, reply) => {
    ...
    if (!Array.isArray(targets) || targets.length < 1 || targets.length > 2) {
      return reply.code(400).send({ error: 'Нужно выбрать одного или двух игроков' });
    }
    const a = String(targets[0]);
    const b = targets.length >= 2 ? String(targets[1]) : null;
    if (b && a === b) return reply.code(400).send({ error: 'Выберите разных игроков' });
    if (a === String(playerId) || (b && b === String(playerId))) {
    ...
    gs.roleSetupVotes[playerId] = b ? [targets[0], targets[1]] : [targets[0]];
    const io = fastify.io;
    io.to(roomId).emit('mafia_phase', { phase: gs.phase, roleSetupTick: true });
    const poolIds = gs.alive || [];
    const voted = poolIds.filter((id) => gs.roleSetupVotes[id] != null).length;
    if (poolIds.length > 0 && voted >= poolIds.length) {
      const fin = finalizeMafiaRoleVotePhase(roomId, gs, io);
      if (fin.error) return reply.code(400).send({ error: fin.error });
    }
    return { ok: true };
  });
  ...
  fastify.post('/rooms/:roomId/mafia/set-roles', async (request, reply) => {
    const { roomId } = request.params;
    const { playerId, donId, mafiaId, mafiaIds, commissionerId, doctorId, prostituteId } = request.body || {};
    ...
    const r = assignRolesFromModeratorPicks({
      poolIds,
      donId,
      mafiaIds: Array.isArray(mafiaIds) ? mafiaIds : undefined,
      mafiaId,
      commissionerId,
      doctorId,
      prostituteId,
  ...
  fastify.post('/rooms/:roomId/mafia/advance', async (request, reply) => {
    ...
    if (gs.phase === 'prep_day_1') {
      const mode = gs.settings?.mafiaRolesMode;
      const hasRoles = gs.roles && Object.keys(gs.roles).length > 0;
      if (!hasRoles && mode === 'moderator') {
        markMafiaPhase(gs, 'role_setup_moderator');
        io.to(roomId).emit('mafia_phase', { phase: gs.phase });
        io.to(roomId).emit('room_updated');
        return { ok: true };
      }
      if (!hasRoles && mode === 'player_vote') {
        markMafiaPhase(gs, 'role_setup_vote');
        io.to(roomId).emit('mafia_phase', { phase: gs.phase });
        io.to(roomId).emit('room_updated');
        return { ok: true };
      }
    ...
    if (gs.phase === 'night_meet') {
      markMafiaPhase(gs, 'prep_day_2');
      io.to(roomId).emit('mafia_phase', { phase: gs.phase });
      return { ok: true };
    }
    if (gs.phase === 'prep_day_2') {
      gs.killNightEnabled = true;
    ...
    if (gs.phase === 'dawn_kill' || gs.phase === 'dawn_doctor' || gs.phase === 'dawn_prostitute') {
      const getName = (id) => room.players.find((p) => String(p.id) === String(id))?.name || id;
      if (gs.phase === 'dawn_kill') {
        ...
        if (nextAfterKill === 'day') {
          clearDawnPending(gs);
          markMafiaPhase(gs, 'day');
        } else {
          markMafiaPhase(gs, nextAfterKill);
        }
      } else if (gs.phase === 'dawn_doctor') {
        applyDawnDoctorAnnounce(gs, getName);
        const nextAfterDoc = nextDawnPhaseAfterDoctor(gs);
        if (nextAfterDoc === 'day') {
          clearDawnPending(gs);
          markMafiaPhase(gs, 'day');
        } else {
          markMafiaPhase(gs, nextAfterDoc);
        }
      } else {
        applyDawnProstituteAnnounce(gs, getName);
        clearDawnPending(gs);
        markMafiaPhase(gs, 'day');
      }
    ...
    if (gs.phase === 'voting') {
      const r = applyMafiaDayVotingResolution(roomId, room, gs, io);
      if (r.winner) return { ok: true, winner: r.winner };
      return { ok: true };
    }
  ...
  fastify.post('/rooms/:roomId/mafia/vote', async (request, reply) => {
    ...
    const io = fastify.io;
    io.to(roomId).emit('mafia_phase', { phase: gs.phase, voteTick: true });
    return { ok: true };
  });

  /** Ведущий: пропустить дневное голосование (никого не исключают, если голосов не было — как advance). */
  fastify.post('/rooms/:roomId/mafia/skip-voting', async (request, reply) => {
    const { roomId } = request.params;
    const { playerId } = request.body || {};
    if (!playerId) return reply.code(400).send(ERR.playerIdRequired);
    if (!allowAction(`mafia:skipvote:${roomId}:${playerId}`, 6, 15_000)) {
      return reply.code(429).send(ERR.tooMany);
    }
    const room = roomManager.get(roomId);
    if (!room || room.game !== 'mafia' || !room.gameState) return reply.code(404).send({ error: 'Game not found' });
    const gs = room.gameState;
    if (String(gs.moderatorId) !== String(playerId)) {
      return reply.code(403).send({ error: 'Только ведущий может пропустить голосование' });
    }
    if (gs.phase !== 'voting') return reply.code(400).send({ error: 'Сейчас не фаза голосования' });
    const io = fastify.io;
    const r = applyMafiaDayVotingResolution(roomId, room, gs, io);
    if (r.winner) return { ok: true, winner: r.winner };
    return { ok: true };
  ...
  fastify.post('/rooms/elias/start', async (request, reply) => {
    ...
    const gs = {
      ...
      playerStats: {},
      /** Слова текущего раунда: { word, outcome, awardedToTeam?, lastWordBonus? } */
      roundLog: [],
  ...
  fastify.get('/rooms/:roomId/elias/state', async (request, reply) => {
    ...
    return {
      ...
      preRoundReadyIds: Array.isArray(gs.eliasPreRoundReady)
        ...
        : [],
      /** Сколько человек должны нажать «Готов» перед раундом (Элиас: всегда 1 — только объясняющий) */
      preRoundRequiredCount: gs.awaitingExplainerStart ? 1 : null,
      roomPlayersCount: room.players.length,
      playerStats,
      mvp,
      roundPhase,
      roundLog: Array.isArray(gs.roundLog) ? gs.roundLog : [],
      dictionaryIds: gs.dictionaryIds || ['basic'],
      roundStartScores: Array.isArray(gs.roundStartScores) ? gs.roundStartScores : null,
    };
  });

  function recomputeEliasScoresFromRoundLog(gs, roundLog) {
    const base = gs.roundStartScores;
    if (!base || !Array.isArray(roundLog)) return;
    const ci = gs.currentTeamIndex;
    const penalty = Math.min(3, Math.max(0, Number(gs.skipPenalty) || 0));
    for (let i = 0; i < gs.teams.length; i++) {
      gs.teams[i].score = Number(base[i]) || 0;
    }
    for (const e of roundLog) {
      if (e.outcome === 'guessed') {
        if (typeof e.awardedToTeam === 'number' && gs.teams[e.awardedToTeam]) {
          gs.teams[e.awardedToTeam].score = (gs.teams[e.awardedToTeam].score || 0) + 1;
        } else {
          gs.teams[ci].score = (gs.teams[ci].score || 0) + 1;
        }
      } else if (e.outcome === 'skipped') {
        if (penalty > 0 && !e.lastWord) {
          gs.teams[ci].score = Math.max(0, (gs.teams[ci].score || 0) - penalty);
        }
      }
    }
  }
  ...
  function eliasNextWord(roomId, io) {
    ...
    if (gs.roundPhase && gs.roundPhase !== 'playing') return;
  ...
  /** Старт раунда после «Готов» у текущего объясняющего. */
  ...
  fastify.post('/rooms/:roomId/elias/guessed', async (request, reply) => {
    ...
    if (gs.roundPhase !== 'playing') {
      return reply.code(400).send({ error: 'Сейчас нельзя отметить слово' });
    }
    if (gs.lastGuessedWord === gs.currentWord) return { ok: true, already: true };
    gs.lastGuessedWord = gs.currentWord;
    if (!Array.isArray(gs.roundLog)) gs.roundLog = [];
    gs.roundLog.push({ word: gs.currentWord, outcome: 'guessed' });
    recomputeEliasScoresFromRoundLog(gs, gs.roundLog);
  ...
  fastify.post('/rooms/:roomId/elias/skip', async (request, reply) => {
    ...
    if (gs.roundPhase !== 'playing') {
      return reply.code(400).send({ error: 'Сейчас нельзя пропустить слово' });
    }
    if (gs.lastSkippedWord === gs.currentWord) return { ok: true, already: true };
    gs.lastSkippedWord = gs.currentWord;
    if (!Array.isArray(gs.roundLog)) gs.roundLog = [];
    gs.roundLog.push({ word: gs.currentWord, outcome: 'skipped' });
    recomputeEliasScoresFromRoundLog(gs, gs.roundLog);
    const stats = gs.playerStats?.[playerId] || { guessed: 0, skipped: 0 };
  ...
  /** Перед раундом: «Готов» только у текущего объясняющего; остальные команды не участвуют. */
  fastify.post('/rooms/:roomId/elias/ready-pre-round', async (request, reply) => {
    ...
    if (!gs.awaitingExplainerStart) {
      return { ok: true, already: true, readyCount: 0, totalPlayers: 1, started: true };
    }
    const teamNow = gs.teams[gs.currentTeamIndex];
    const explainerId = teamNow?.players?.length
      ? teamNow.players[gs.currentExplainerIndex % teamNow.players.length]
      : null;
    if (explainerId == null) {
      return reply.code(400).send({ error: 'Нет текущего объясняющего' });
    }
    if (String(playerId) !== String(explainerId)) {
      return reply.code(403).send({ error: '«Готов» перед раундом нажимает только текущий объясняющий' });
    }
    const sid = String(playerId);
    gs.eliasPreRoundReady = [sid];
    let started = eliasBeginRoundInternal(roomId, fastify.io);
    if (!started) {
  ...
  fastify.post('/rooms/:roomId/elias/timer-ended', async (request, reply) => {
    ...
    const { playerId } = request.body || {};
    if (!playerId) return reply.code(400).send(ERR.playerIdRequired);
    if (!allowAction(`elias:timeup:${roomId}:${playerId}`, 8, 5000)) {
      return reply.code(429).send(ERR.tooMany);
    }
    const room = roomManager.get(roomId);
    if (!room || room.game !== 'elias' || !room.gameState) return reply.code(404).send({ error: 'Игра не найдена' });
    ...
    if (!gs.roundEndsAt || Date.now() < gs.roundEndsAt - 400) {
      return reply.code(400).send({ error: 'Таймер ещё не истёк' });
    }
    gs.roundPhase = 'last_word';
    gs.roundEndsAt = null;
    fastify.io.to(roomId).emit('elias_update', {});
    return { ok: true };
  });

  fastify.post('/rooms/:roomId/elias/last-word-assign', async (request, reply) => {
    const { roomId } = request.params;
    const { playerId, teamIndex } = request.body || {};
    if (!playerId || teamIndex === undefined || teamIndex === null) {
      return reply.code(400).send({ error: 'Нужны playerId и teamIndex' });
    }
    if (!allowAction(`elias:lastword:${roomId}:${playerId}`, 8, 4000)) {
      return reply.code(429).send(ERR.tooMany);
    }
    const room = roomManager.get(roomId);
    if (!room || room.game !== 'elias' || !room.gameState) return reply.code(404).send({ error: 'Игра не найдена' });
    ...
    if (ti === -1) {
      ...
      recomputeEliasScoresFromRoundLog(gs, gs.roundLog);
      gs.currentWord = null;
      gs.roundPhase = 'review';
      eliasCheckWin(roomId, fastify.io);
      fastify.io.to(roomId).emit('elias_update', {});
      return { ok: true };
    }
    if (ti < 0 || ti >= gs.teams.length) {
      return reply.code(400).send({ error: 'Некорректная команда' });
    }
    if (!Array.isArray(gs.roundLog)) gs.roundLog = [];
    gs.roundLog.push({
  ...
  fastify.post('/rooms/:roomId/elias/finalize-round', async (request, reply) => {
    const { roomId } = request.params;
    const { playerId, roundLog: clientLog } = request.body || {};
    if (!playerId) return reply.code(400).send({ error: 'Нужен playerId' });
    if (!allowAction(`elias:finalize:${roomId}:${playerId}`, 6, 8000)) {
      return reply.code(429).send(ERR.tooMany);
    }
    const room = roomManager.get(roomId);
    if (!room || room.game !== 'elias' || !room.gameState) return reply.code(404).send({ error: 'Игра не найдена' });
    const gs = room.gameState;
    if (gs.roundPhase !== 'review') {
      return reply.code(400).send({ error: 'Нет экрана проверки раунда' });
    }
    const team = gs.teams[gs.currentTeamIndex];
    const explainerId = team?.players?.length
      ? team.players[gs.currentExplainerIndex % team.players.length]
    ...
    if (!Array.isArray(clientLog) || clientLog.length === 0) {
      return reply.code(400).send({ error: 'Нужен список слов' });
    }
    const sanitized = clientLog.map((e) => ({
      word: String(e?.word || '').trim(),
      outcome: e?.outcome === 'skipped' ? 'skipped' : 'guessed',
      awardedToTeam: typeof e?.awardedToTeam === 'number' ? e.awardedToTeam : undefined,
      lastWordBonus: Boolean(e?.lastWordBonus),
      lastWord: Boolean(e?.lastWord),
    })).filter((e) => e.word);
    if (!sanitized.length) return reply.code(400).send({ error: 'Пустой список' });
    gs.roundLog = sanitized;
    recomputeEliasScoresFromRoundLog(gs, gs.roundLog);
    eliasCheckWin(roomId, fastify.io);
    if (gs.winner != null) {
      fastify.io.to(roomId).emit('elias_update', {});
      return { ok: true, winner: gs.winner };
    }
    eliasNextTurn(roomId, fastify.io);
    return { ok: true };
  });
